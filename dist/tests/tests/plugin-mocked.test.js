...
test('plugin tools can be created without store initialization', async (t) => {
    ...
    // Test tool execution
    const compileResult = await tools['compile-context'].execute({ query: 'test', limit: 50 }, mockPluginContext);
    t.true(compileResult.includes('test'));
    t.true(compileResult.includes('Compiled Context'));
    const searchResult = await tools['search-context'].execute({ query: 'test', limit: 20 }, mockPluginContext);
    t.true(searchResult.includes('test'));
    t.true(searchResult.includes('Search Results'));
    const sessionResult = await tools['get-session'].execute({ sessionId: 'session-123' }, mockPluginContext);
...
//# sourceMappingURL=plugin-mocked.test.js.map