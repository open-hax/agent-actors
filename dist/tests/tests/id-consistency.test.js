...
test('message IDs are consistent across tools', async (t) => {
    ...
    const listResult = await listMessagesTool.execute({ sessionId: testSessionId, limit: 10 }, mockPluginContext);
    const listData = JSON.parse(listResult);
...
test('session IDs from spawn-session work with other tools', async (t) => {
    ...
    t.is(getData.session.id, spawnData.session.id, 'get-session should return same spawned session ID');
});
...
test('ID validation works consistently across all tools', async (t) => {
    ...
    for (const { id, expectedMessage } of invalidCases) {
        for (const testTool of testTools) {
            await t.throwsAsync(testTool.execute({ sessionId: id }, mockPluginContext), { message: expectedMessage }, `All tools should reject invalid session ID (${id}) consistently`);
        }
    }
});
...
//# sourceMappingURL=id-consistency.test.js.map