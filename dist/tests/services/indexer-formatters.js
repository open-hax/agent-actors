...
export const eventToMarkdown = (event) => {
...
export const sessionToMarkdown = (session) => `# Session: ${session.title || session.id}
...
---
`;
export const messageToMarkdown = (message) => {
    ...
    const textParts = messageData.parts?.filter((part) => part.type === 'text') ?? [];
