...
export async function initializeStores() {
    ...
    try {
        ...
        const sessionCollection = await DualStoreManager.create(SESSION_STORE_NAME, 'text', 'timestamp');
        const eventCollection = await DualStoreManager.create(EVENT_STORE_NAME, 'text', 'timestamp');
        const messageCollection = await DualStoreManager.create(MESSAGE_STORE_NAME, 'text', 'timestamp');
        return {
